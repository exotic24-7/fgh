    ...
    window._spawnInterval = setInterval(()=>{
        try{
            ...
            if(s.tpl){
                ...
                const size = Math.max(8, Math.round((tpl.baseSize||12) * (1 + rarityIndex*0.14)));
                ...
                if((typeVal||'').toLowerCase() === 'centipede'){
                    ...
                    const mobObj = {x:s.x,y:s.y,radius:size,speed:speed,health:hp,maxHealth:hp,name:tpl.name,type:typeVal,projectiles:[],shootCooldown: shootCd,rarityIndex:rarityIndex,rarityName:rarityName,stationary:!!tpl.stationary, mass: Math.max(1, Math.round(size * (1 + rarityIndex*0.14))), vx:0, vy:0};
                ...
                else if(choice<0.75) mobs.push({x:s.x,y:s.y,radius:Math.round(12 * (1 + rIdx*0.14)),speed:Math.max(0.2,1.2/(Math.max(1,mult*0.8))),health:Math.round(40*mult),maxHealth:Math.round(40*mult),name:"Hornet",type:"Hornet",projectiles:[],shootCooldown:120,spriteKey:'hornet',rarityIndex:rIdx,rarityName:rName,stationary:false, mass: Math.round(12 * (1 + rIdx*0.14)), vx:0, vy:0});
...
function drawProjectiles(){
    projectiles.forEach(p=>{
        if(p.type === 'Missile'){
            ...
            const sc = Math.max(0.6, (p.radius || 4) / 6);
            drawHornetMissile(ctx, p.x, p.y, angle, sc);
            // extra debug outline so missiles are obvious during testing
            ctx.save(); ctx.strokeStyle='magenta'; ctx.lineWidth=1; ctx.beginPath(); ctx.arc(p.x,p.y,Math.max(2, (p.radius||4)),0,Math.PI*2); ctx.stroke(); ctx.restore();
        } else {
            ctx.fillStyle = 'white';
    ...
    // log projectile counts at most once per second for debugging
    try{
        window._lastProjLog = window._lastProjLog || 0;
        if(Date.now() - window._lastProjLog > 1000){ console.log('projectiles:', projectiles.length, 'recent types:', projectiles.slice(-6).map(p=>p.type)); window._lastProjLog = Date.now(); }
    }catch(e){}
    // draw temporary debug markers for recently emitted projectiles (visual aid)
    if(window._projDebug && window._projDebug.length){
        const now = Date.now();
        for(let i = window._projDebug.length - 1; i >= 0; i--){
            const d = window._projDebug[i];
            if(!d || now > d.until){ window._projDebug.splice(i,1); continue; }
            const a = Math.max(0, Math.min(1, (d.until - now) / 1200));
            ctx.save(); ctx.globalAlpha = 0.9 * a; ctx.fillStyle = 'rgba(255,80,80,0.9)'; ctx.beginPath(); ctx.arc(d.x,d.y,6 + (1-a)*8,0,Math.PI*2); ctx.fill(); ctx.restore();
        }
    }
}
...
function moveMobs(){
    ...
    mobs.forEach(mob=>{
        ...
        if(t === "hornet"){
            ...
            // Shooting cooldown: use internal _shootTimer and emit into global projectiles
            mob._shootTimer = (typeof mob._shootTimer === 'number') ? mob._shootTimer - 1 : ((mob.shootCooldown || 120) - 1);
            if(mob._shootTimer <= 0){
                // indicate a longer turn animation so the smooth rotation can complete
                mob._turnUntil = Date.now() + 600;
                let angle = Math.atan2(player.y-mob.y, player.x-mob.x);
                // push into global projectiles so missiles behave like player projectiles
                const spawnOffset = (mob.radius || 12) + 8;
                projectiles.push({x:mob.x + Math.cos(angle) * spawnOffset, y:mob.y + Math.sin(angle) * spawnOffset, dx:Math.cos(angle)*4, dy:Math.sin(angle)*4, radius:6, type:"Missile", damage:5, _owner: mob});
                // debug log to confirm hornet fired (remove when verified)
                try{ console.log('Hornet fired at', mob.x.toFixed(1), mob.y.toFixed(1), 'angle=', angle.toFixed(2)); }catch(e){}
                // temporary visual debug marker for recent projectiles
                try{ window._projDebug = window._projDebug || []; window._projDebug.push({x:mob.x,y:mob.y,until: Date.now()+1200}); }catch(e){}
                mob._shootTimer = mob.shootCooldown || 120; // reset timer
            }
        } else {
        ...
        try{
            ...
            const desired = (mob._turnUntil && mob._turnUntil > Date.now()) ? playerAngle : (playerAngle + Math.PI);
            if(typeof mob._displayAngle !== 'number') mob._displayAngle = desired;
            let diff = desired - mob._displayAngle;
            while(diff > Math.PI) diff -= Math.PI * 2;
            while(diff < -Math.PI) diff += Math.PI * 2;
            const maxStep = (mob._turnUntil && mob._turnUntil > Date.now()) ? 0.28 : 0.06;
...
function checkCollisions(){
    ...
    /* --- Global projectiles ↔ Player (missiles and enemy projectiles) --- */
    for(let pi = projectiles.length - 1; pi >= 0; pi--){
        const proj = projectiles[pi];
        // Missiles (fired by hornets) should hit the player, not mobs
        if(proj.type === 'Missile'){
            const d = Math.hypot(player.x - proj.x, player.y - proj.y);
            if(d < player.radius + (proj.radius || 4)){
                if(!player.godmode) player.health -= (proj.damage || 1);
                player._hitFlash = Date.now();
                // apply knockback
                const pm = proj.mass || 0.6;
                const nx = (player.x - proj.x) / Math.max(0.0001, d);
                const ny = (player.y - proj.y) / Math.max(0.0001, d);
                player.vx = (player.vx || 0) + nx * (pm / player.mass) * 8;
                player.vy = (player.vy || 0) + ny * (pm / player.mass) * 8;
    ...
    for(let pj = pi; pj >= 0; pj--){
        // reuse loop variable space: pj will iterate remaining projectiles indices
    }
    // NOTE: fallthrough to existing logic below will handle non-missile projectiles
    // We need to continue into the original loop body; to avoid restructuring large block,
    // we'll re-enter the normal projectile->mob loop by resetting pi to projectiles.length-1
    // and letting the existing code run. (No further change needed here.)
    // Reset iterator so the following block processes non-missile projectiles as before.
    pi = projectiles.length - 1;
    if(pi < 0) {
        // nothing left, skip below
    }
    
    // The code below expects the for-loop variable `pi` to be defined; to preserve that,
    // we simply fall through to the existing `for` loop which immediately follows.

    // (end of missile->player handling)
    ...
    for(; pi >= 0; pi--){
        ...
        for(let mi = mobs.length - 1; mi >= 0; mi--){
            ...
            // skip collision against the projectile's owner for the first tick
            if(proj._owner && proj._owner === mob) continue;
            // missiles should not hit mobs
            if(proj.type === 'Missile') continue;
